...
const (
	...
	lbPriorityClassName                 = "ibm-app-cluster-critical" // #nosec G101 Just the name of the LB Priority Class, not an actual secret
	...
	for i := range services.Items {
		if 0 != len(services.Items[i].Status.LoadBalancer.Ingress) &&
			...
			if nil == lbDeployment || nil != err {
				...
				if isEventRequired {
					// #nosec G104 No error handling needed as event is being recrded by Recorder.
